...
/* const aaa56Price = flightReservations.find((f) => f.flightNumber == "AA456");
console.log("El precio total de vuelo número 'AA456' es:", aaa56Price.totalPrice); */
// Usa el método find para encontrar el vuelo que ha reservado el señor bob.johnson@example.com. Muestra el objeto entero

/* const vueloDeSenor = flightReservations.find((f) => f.passenger.contactInfo.email == "bob.johnson@example.com");
console.log('Vuelo de senor Bob Johnson :', vueloDeSenor); */

// Usa el método some para averiguar si algún vuelo tiene como destino el aeropuerto de LPA GRAN CANARIA

/* const destinoGranCanaria = flightReservations.some((f) => f.arrival.airport == "LPA GRAN CANARIA");
console.log("Ejiste vuelo a Gran Canaria? ", destinoGranCanaria); */
// Usa el método every para comprobar si todos los vuelos están confirmados (isConfirmed)
/* const isConfirmed = flightReservations.every((f) => f.isConfirmed = true);
console.log('todos los vuelos están confirmados (isConfirmed)?', isConfirmed); */

// Usa el método filter para obtener todos los vuelos que tienen la puerta de embarque 'D5'
/* const puertaDeEmbarque = flightReservations.filter((f) => f.gate == "D5");
console.log("Los vuelos con la puerta de embarque 'D5'", puertaDeEmbarque);
 */
...
/* const menuVegan = flightReservations.filter((f) => f.specialMeals.includes("Vegan"));
menuVegan.forEach((reservation) => {
	console.log('Aerolinea con menu "Vegan" es: ', reservation.airline)
});
 */
...
const vuelos = flightReservations.map((f) => f.flightNumber + "-" + f.airline);
console.log(vuelos);
// DIFICIL. USA el método reduce para sumar el conjunto total de puntos obtenidos de loyalyProgram de todos los tickets
const puntosObtenidos = flightReservations.reduce((total, f) => { return total + f.loyaltyProgram.points }, 0);
/* {
	(total, num) =>
		total + num;
